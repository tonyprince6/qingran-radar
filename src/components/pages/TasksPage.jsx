...
export default function TasksPage({ data, collecting, onRun }) {
  ...
  return <div className="tasks-layout">
    <section className="task-hero">
      ...
      <h2>话题每 4 小时 · 视频互动每 20 分钟</h2>
      <p>先补全视频链接，再刷新点赞、评论、收藏和分享；仅跟踪“减脂、减肥、体重管理”。</p>
    ...
    <div className="task-columns">
      <section className="page-panel task-config"><div className="section-title"><h2>任务配置</h2><span>ACTIVE</span></div>
        <dl><div><dt>数据源</dt><dd>{source?.name ?? '抖音创作者中心'}</dd></div><div><dt>话题发现</dt><dd>每 4 小时</dd></div><div><dt>互动刷新</dt><dd>每 20 分钟</dd></div><div><dt>关键词</dt><dd>{data?.keywords?.join('、') ?? '减脂、减肥、体重管理'}</dd></div><div><dt>处理方式</dt><dd>补链接 → 读取互动 → 记录24h增长 → 更新网站</dd></div></dl>
